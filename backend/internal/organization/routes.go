...
func RegisterRoutes(router fiber.Router, h *Handler, mw ...fiber.Handler) {
	if len(mw) > 0 && mw[0] != nil {
		router.Post("/organizations", mw[0], h.Create)
		router.Get("/organizations", mw[0], h.List)
		router.Get("/organizations/:id", mw[0], h.GetByID)
		router.Put("/organizations/:id", mw[0], h.Update)
		router.Delete("/organizations/:id", mw[0], h.Delete)
		router.Get("/organizations/:id/members", mw[0], h.GetMembers)
		router.Get("/organizations/:id/members/me", mw[0], h.GetMember)
		router.Put("/organizations/:id/members/:userID/role", mw[0], h.UpdateMemberRole)
		router.Delete("/organizations/:id/members/:userID", mw[0], h.RemoveMember)
		router.Post("/organizations/:id/invitations", mw[0], h.Invite)
		router.Get("/organizations/:id/invitations", mw[0], h.ListInvitations)
		router.Post("/invitations/:id/accept", mw[0], h.AcceptInvitation)
		router.Post("/invitations/:id/decline", mw[0], h.DeclineInvitation)
		router.Delete("/invitations/:id", mw[0], h.CancelInvitation)
		return
	}
